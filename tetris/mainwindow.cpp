...
{
    ui->setupUi(this);
    isGame9ButtonVisible = false;  // 初始化按钮为隐藏状态
    ui->Game9Button->hide();  // 隐藏9人乱斗按钮
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->key() == Qt::Key_O && event->modifiers() == Qt::ControlModifier) {
        isGame9ButtonVisible = !isGame9ButtonVisible;  // 切换按钮状态
        ui->Game9Button->setVisible(isGame9ButtonVisible);  // 显示或隐藏按钮
    }
    switch (event->key())
    {
        case Qt::Key_Enter:
